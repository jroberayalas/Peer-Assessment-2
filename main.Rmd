...
Apart from the numbers 0-8, there are a couple of letters and symbols. We transform these using the following code, which create a new variable called **TOTAL.COST.PROP**:
...
```{r plot1}
fatalitiesPlot <- ggplot(data = fatalities, aes(x = reorder(EVTYPE, count), y = count)) + 
...
injuriesPlot <- ggplot(data = injuries, aes(x = reorder(EVTYPE, count), y = count)) + 
    geom_bar(stat = "identity", aes(fill = count)) +
    scale_fill_continuous(low = "blue", high = "red") +
    coord_flip() +
